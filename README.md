...
- **비용**은 체크리스트와 완전히 독립된 목록입니다. 관리 화면에서 "내용 + 금액 + 낸 사람"을 입력해 비용을 추가하면, 참여자가 공유 링크에서 체크리스트 항목처럼 체크해서 "이 비용을 나눠 낼 사람"을 정합니다. 그 비용을 체크한 사람들끼리 금액을 똑같이 나누고, 모든 비용을 합산해 "누가 얼마 받아야/내야 하는지"와 최소 이동 횟수로 정리한 "A → B 금액" 정산표를 관리 화면과 참여자 화면 양쪽에 자동으로 보여줍니다.
