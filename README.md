...
- 항목 수정에서 **금액**과 **낸 사람**을 입력하면 비용 정산 기능이 켜집니다. 그 항목을 체크한 사람들끼리 금액을 똑같이 나눠서, "누가 얼마 받아야/내야 하는지"와 최소 이동 횟수로 정리한 "A → B 금액" 정산표를 관리 화면과 참여자 화면 양쪽에 자동으로 보여줍니다. 마크다운 가져오기로는 금액/낸 사람을 지정할 수 없고, 항목별 "수정"에서만 설정할 수 있습니다.
